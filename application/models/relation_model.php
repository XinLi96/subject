...
class Relation_model extends CI_Model{
    ...
    public function get_course_by_id_week($user_id,$week){//学生通过用户、上课的周数以周的方式查看课程表
        $sql = 'select * from relation r where r.user_id = '.$user_id.' and r.begin<='.$week.' and r.end>='.$week.'';
        $query = $this->db->query($sql);
        return $query->result();
    }
    public function get_course_by_id_day($user_id,$week,$day){//学生通过用户、上课的周数、上课是星期几以天的方式查看课程表
